...
只返回 JSON 数组，不包含任何其他文字。每项必须包含 entity 字段指定要编辑的角色 id。格式：
  - 设置字符串（文本类字段）：{"op": "set-string", "entity": "角色id", "path": "状态.心情", "value": "愉快"}
  - 数值增减（数字类字段，delta 为正数表示增加、负数表示减少）：{"op": "adjust-number", "entity": "角色id", "path": "属性.体力", "delta": -5}
  - 追加到列表：{"op": "push-to-list", "entity": "角色id", "path": "日志.事件", "value": "触发了一段对话"}

### 重要规则
- 数值类型的字段**必须**用 adjust-number，**禁止**用 set-string 写入数字，否则会破坏类型
- 这些工具也同时被用于新建对应类型的数据
- 如果当前对话没有引起任何字段需要变化，返回空数组 []，**不要**无中生有编造编辑
- path 按 "类别.子类别.字段名" 的规律组织，遇到尚未存在的路径时按结构自动创建中间节点——如果不存在，数值字段通过adjust-number，从 0 开始累加，字符串字段用 set-string 直接写入
- 只修改确实因本轮对话而变化的字段，已有的合理数值不要重复写入`
